...
if (!openaiApiKey) {
  throw new Error(
    "OPENAI_API_KEY no está definida en Firebase Functions. Verifica tu configuración."
  );
}
...
app.get("/api/trainner", async (req, res) => {
  ...
  if (!training_type || !duration) {
    return res
      .status(400)
      .json({ error: "Faltan parámetros 'training_type' o 'duration'" });
  }
...
// Exportar la función como un endpoint HTTP de Firebase Functions
